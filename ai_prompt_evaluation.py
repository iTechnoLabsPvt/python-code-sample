...
def getFilesFromType(sourceDir: str, filetype: str) -> list[str]:
    """
    Recursively finds all files with the specified file type in the given directory.

    Args:
        sourceDir (str): The directory to search in.
        filetype (str): The file type to search for (e.g. ".py").

    Returns:
        list[str]: A list of file paths matching the specified file type.
    """
    files = []
...
def get_code(directory: str) -> str:
    """
    Retrieves the code from all Python files in the given directory.

    Args:
        directory (str): The directory containing the Python files.

    Returns:
        str: The concatenated code from all Python files, formatted with Markdown code blocks.
    """
...
def get_completeness(directory: str) -> float:
    """
    Calculates the completeness score of the code in the given directory.

    Args:
        directory (str): The directory containing the code.

    Returns:
        float: The completeness score, ranging from 0.0 (incomplete) to 1.0 (complete).
    """
...
def get_executability(directory: str) -> float:
    """
    Checks the executability of the code in the given directory.

    Args:
        directory (str): The directory containing the code.

    Returns:
        float: The executability score, 1.0 if the code runs successfully, 0.0 otherwise.
    """
    assert os.path.isdir(directory)

    def findFile(directory: str, target: str) -> str | None:
        main_py_path = None
...
def get_consistency(directory: str) -> float:
    """
    Calculates the consistency score between the code and the task description in the given directory.

    Args:
        directory (str): The directory containing the code and task description.

    Returns:
        float: The consistency score, ranging from 0.0 (inconsistent) to 1.0 (consistent).
    """
    ...
    def get_cosine_similarity(
        embeddingi: list[float], embeddingj: list[float]
    ) -> float:
...
def main(warehouse_root: str):
    """
    The main function that processes all directories in the warehouse root.

    Args:
        warehouse_root (str): The root directory containing the directories to process.
    """
